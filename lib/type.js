...
WidgetType.prototype.debounceWait = 500;
...
// Filter a match
WidgetType.prototype.filterMatch = function(range, match) {
    return true;
};
...
WidgetType.prototype.cleanupMarkers = function(origin) {
    ...
    _.each(this.doc().getAllMarks(), function(mark) {
        ...
        var isInRange = utils.isPos(markRange)? utils.posInsideRange(markRange, origin) : utils.rangesOverlap(markRange, origin);
        if (!isInRange) return;
...
WidgetType.prototype.processChange = function(change) {
    ...
    // Find range that have to re-rendering
    changeRange = this.findEditRange(changeRange);

    // Determine large range and cleanup markers in it
    changeRange = this.cleanupMarkers(changeRange);

    // Update markers in this range
    this.processRange(changeRange);
};
